/* 아이디·비밀번호 입력 규칙과 그 규칙을 어겼을 때의 안내 문구.
   가입(아이디+비밀번호)·비밀번호 찾기·비밀번호 변경 세 화면이 같은 규칙을 쓰므로 여기 하나만 둔다.
   iOS 는 별도 npm 프로젝트라 ios/src/credentials.ts 에 같은 규칙을 복제한다(useResendCooldown 과 같은 사정).
   규칙을 고치면 두 파일과 scripts/test_credentials.mjs 를 같이 고친다.

   아이디   5~20자 · 영문 대/소문자 + 숫자 + 특수문자 _ - . 만
   비밀번호 5~20자 · 영문 1자 이상 필수 + 숫자 1자 이상 필수, 특수문자는 ! @ # $ % ^ & * ? _ 허용
   둘 다 한글·공백 금지. 대문자와 특수문자는 "허용"이지 필수가 아니다 — 강제하지 않는다.

   문구는 "무엇이 틀렸는지"를 말한다. 규칙 전체를 다시 읊어 주는 안내는 사용자가 자기 입력의
   어디를 고쳐야 하는지 못 찾는다 — 공백·한글·못 쓰는 문자·길이·구성요소를 각각 따로 잡는다. */
...
function badChars(value, allowed) {
