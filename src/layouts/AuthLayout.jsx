...
const LIGHT_ON_VIDEO = {
  ...
  // 비활성·호버 버튼 면(.ds-btn:disabled 등)도 흰 글자 기준으로 뒤집는다 — 라이트 값(#f5f5f6)
  // 그대로면 "발송 중…" 같은 비활성 버튼 글자가 흰 면에 묻힌다.
  "--surface-sunken": "rgba(255,255,255,.12)",
  "--border-subtle": "rgba(255,255,255,.28)",
  // 포커스 표시도 같이 뒤집는다. 프리미티브가 쓰는 원래 값(--focus-border 인디고, 반투명 인디고
  // 링)은 흰 바탕 기준이라 영상 위에서는 평상시 테두리보다 오히려 안 보인다 — 포커스했더니
  // 흐려지는 꼴이 된다.
  "--focus-border": "#ffffff",
