...
| 대상 | 원본 |
|---|---|
| 모바일 앱 5화면 | `handoff/untitled/project/청원시스템 Mobile.dc.html` (721줄 — 1–435행 화면 마크업, 436–716행 상태·데이터·핸들러). **⚠ 2026-07-29 현재 이 파일이 디스크에 없다** — 이 문서와 이미 만든 화면 코드가 그 값의 유일한 사본이다. M3-2 픽셀 대조는 원본 없이는 성립하지 않는다 |
...
서로 일치함을 확인했다. **모바일 프로토타입 HTML 은 디스크에 없어 쓰지 못했다**(위 스펙 표 참조).
