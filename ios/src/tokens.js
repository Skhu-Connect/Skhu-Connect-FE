...
const colors = {
  ...
  /* 옛 색 계열. 아직 범위 밖 화면(상세·알림 설정)과 옛 그라데이션이 쓰고 있어 남긴 것이다.
...
/* 그라데이션은 새 규칙에서 쓰지 않는다.
   mileage 는 범위 밖 상세 화면의 배너(Detail.tsx)가 아직 쓰므로 남긴다 — 그 화면을 옮길 때 걷는다.
   hero 는 남은 소비자가 피드 히어로와 MY 프로필 머리뿐이라, 그 둘을 옮기는 마지막 회차에서 지운다
   (지금 지우면 두 화면이 undefined 를 펼쳐 런타임에서 깨진다 — theme.ts 가 타입만 입혀
   재수출하므로 타입 검사로는 잡히지 않는다). */
const gradient = {
